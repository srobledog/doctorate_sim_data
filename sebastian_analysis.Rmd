...
egonet_list <- make_ego_graph(sebastian_asn_igraph)

dat <- data.frame(
  name = names(V(sebastian_asn_igraph)),
  egonet_density = lapply(egonet_list, 
                          graph.density) %>% 
    unlist(),
  constraint = lapply(egonet_list, 
                          constraint) %>% 
    unlist()
)
...
sebastian_data <- 
  ...
  activate(nodes) |> 
  as_tibble() |> 
  dplyr::select(egonet_density, 
                total) |> 
  na.omit()

summary(lm(egonet_density ~ total,
           sebastian_data))
```
